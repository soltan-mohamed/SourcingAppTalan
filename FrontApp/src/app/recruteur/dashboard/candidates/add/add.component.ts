    ...
    .subscribe({
      ...
      error: (err) => {
        ...
        if (err.status === 403) {
          ...
          this.showError(`Failed to create candidate: ${err.message}`);
        }
      }
    });
}
...
showSuccess(message: string): void {
  // Replace with Angular Material Snackbar or other UI feedback
  alert(message);
}
    ...
    .subscribe({
      next: (event) => {
        ...
        if (event.type === 4) { // HttpResponse
          this.showSuccess('Candidate and CV uploaded successfully');
...
validateFile(file: File): void {
  ...
  this.fileInput.nativeElement.value = ''; // Reset input to allow same file re-selection
}
...
removeKeyword(keyword: string): void {
  ...
  getFormProgress(): number {
    ...
    if (this.selectedFile) progress += 20;
